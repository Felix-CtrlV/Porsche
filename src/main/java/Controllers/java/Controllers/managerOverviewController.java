...
import Controllers.bestSellerController;
import Controllers.managerCarPartProgressBarController;
...
public class managerOverviewController {
    ...
    private void applySmoothCurves() {
        ...
        List<javafx.scene.shape.PathElement> smoothedLineElements = new ArrayList<>();
        ...
        linePaths.forEach(node -> {
                ...
                smoothPath(path);
                smoothedLineElements.addAll(new ArrayList<>(path.getElements()));
            }
        });
        ...
        fillPaths.forEach(node -> {
            if (node instanceof javafx.scene.shape.Path && !smoothedLineElements.isEmpty()) {
                ...
                var originalFillElements = new ArrayList<>(fillPath.getElements());
    ...
    private void setChartsDataAsync() throws SQLException {
        ...
        List<XYChart.Data<String, Integer>> carData = new ArrayList<>();
        List<XYChart.Data<String, Integer>> partData = new ArrayList<>();
        List<XYChart.Data<String, Double>> revenueData = new ArrayList<>();
    ...
    private void setBestiDataAsync() throws SQLException, IOException {
        ...
        // Collect data in background thread
        List<managerOverview> carPartItems = new ArrayList<>();
        List<managerOverview> sellerItems = new ArrayList<>();
            ...
            if (besti.equals("car") || besti.equals("part")) {
                for (managerOverview item : bestCarPartList) {
                    try {
                        ...
                        Controllers.managerCarPartProgressBarController controller = loader.getController();
                ...
                for (managerOverview seller : bestSellerList) {
                    try {
                        ...
                        Controllers.bestSellerController controller = loader.getController();
