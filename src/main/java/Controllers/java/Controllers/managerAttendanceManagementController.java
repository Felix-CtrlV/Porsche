...
import Controllers.AddAbsentDialogController;
...
public class managerAttendanceManagementController implements Initializable {
    ...
    private void scrollToDatePicker() {
        ...
        javafx.application.Platform.runLater(() -> {
            try {
                ...
                Scene scene = dateRangeBtn.getScene();
                if (scene != null) {
                ...
                if (currentWindow instanceof Stage) {
                    ...
                    if (ownerStage != null && ownerStage.getScene() != null) {
                        ScrollPane parentScrollPane = findScrollPane(ownerStage.getScene().getRoot());
    ...
    private List<ScrollPane> findAllScrollPanes(javafx.scene.Node node) {
        List<ScrollPane> scrollPanes = new ArrayList<>();
        
        if (node instanceof ScrollPane) {
            scrollPanes.add((ScrollPane) node);
        }
